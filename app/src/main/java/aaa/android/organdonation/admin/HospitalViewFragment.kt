...
import aaa.android.organdonation.adapter.HospitalAdapter
...
class HospitalViewFragment : Fragment(), ListItemClickListener,
    SignUpAcceptListAdapter.SendMailClickListener, HospitalAdapter.DonorItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val adapter = HospitalAdapter(requireActivity(), this)
    ...
    override fun approved(item: Any) {
        val itemData = item as Hospital
        lifecycleScope.launchWhenResumed {
            updateResponse.postValue(itemData.id?.let {
                expenseViewModel.updateHospitalStatusResponse(
                    "Deleted",
                    it
                )
            })
        ...
        lifecycleScope.launchWhenResumed {
            val donorList = expenseViewModel.getHospitalDetails()
            donorList?.let {
                adapter?.setAdapter(it)
            }
        }
